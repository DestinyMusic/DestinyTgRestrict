...
def _ouo(link):
    ...
    try:
        with cffi_requests.Session(impersonate="chrome136", timeout=30) as s:
            ...
            if not tok1:
                raise DirectDownloadLinkException(f"ERROR: ouo: _token not found on initial page (status={r1.status_code})")
            ...
            if r2.status_code != 200:
                raise DirectDownloadLinkException(f"ERROR: ouo: /go/ status {r2.status_code}")
            tok2 = _extract_csrf(r2.text)
            if not tok2:
                raise DirectDownloadLinkException("ERROR: ouo: _token not found on /go/ page")
            ...
            if r3.status_code != 302 or not location:
                raise DirectDownloadLinkException(f"ERROR: ouo: /xreallcygo/ status {r3.status_code} (location={location!r})")
            return location
