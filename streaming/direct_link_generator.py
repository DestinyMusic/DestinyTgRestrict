...
class Config:
    DEBRID_LINK_API = os.environ.get("DEBRID_LINK_API", "")
...
PASSWORD_ERROR_MESSAGE = "Password required for {}"

def is_share_link(url):
    return any(x in url for x in ["filepress", "filebee", "sharer"])
...
def speed_string_to_bytes(size_str):
    ...
    if not match:
        return 0
    val, unit = float(match.group(1)), (match.group(2) or "b").lower()
    units = {"b": 1, "k": 1024, "kb": 1024, "m": 1024**2, "mb": 1024**2, "g": 1024**3, "gb": 1024**3, "t": 1024**4, "tb": 1024**4}
    return int(val * units.get(unit, 1))

from streaming.url_shortener_bypass import bypass_shortener, is_url_shortener
